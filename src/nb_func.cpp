...
void nb_func_dealloc(PyObject *self) {
    ...
    if (size) {
        ...
        size_t n_deleted = ((nb_func *) self)->internals->funcs.erase(self);
        check(n_deleted == 1,
    ...
    nb_internals *p = ((nb_func *) self)->internals;
...
nb_func_error_overload(PyObject *self, PyObject *const *args_in,
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        buf.put_uint32(i + 1);
        buf.put(". ");
        nb_func_render_signature(buf, ((nb_func *) self)->internals, f + i);
        buf.put('\n');
    }
...
static NB_NOINLINE PyObject *nb_func_error_noconvert(PyObject *self,
    ...
    nb_func_render_signature(buf, ((nb_func *) self)->internals, f);
    PyErr_SetString(PyExc_TypeError, buf.get());
    return nullptr;
}

/// Used by nb_func_vectorcall: convert a C++ exception into a Python error
static NB_NOINLINE void nb_func_convert_cpp_exception(nb_internals *p) noexcept {
    ...
    for (size_t pass = (count > 1) ? 0 : 1; pass < 2; ++pass) {
        for (size_t k = 0; k < count; ++k) {
            ...
            try {
                ...
                nb_func_convert_cpp_exception(((nb_func *) self)->internals);
            }
    ...
    if (inst) {
        ...
        nb_bound_method *mb =
            PyObject_GC_New(nb_bound_method,
                            ((nb_func *) self)->internals->nb_bound_method);
    ...
    if ((f->flags & (uint32_t) func_flags::has_scope) &&
        ...
        PyObject *scope_name =
            getattr(((nb_func *) self)->scope,
                    NB_INTERNED(((nb_func *) self)->internals, __qualname__),
                    nullptr);
        if (scope_name) {
...
PyObject *nb_func_get_nb_signature(PyObject *self, void *) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        buf.clear();
        uint32_t n_default_args = nb_func_render_signature(
            buf, ((nb_func *) self)->internals, fi, true);
...
PyObject *nb_func_get_doc(PyObject *self, void *) {
    ...
    for (uint32_t i = 0; i < count; ++i) {
        const func_data *fi = f + i;
        nb_func_render_signature(buf, ((nb_func *) self)->internals, fi);
        buf.put('\n');
