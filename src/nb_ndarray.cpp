...
#define OK_TO_USE_STD_UNIQUE_PTR
#ifdef OK_TO_USE_STD_UNIQUE_PTR
#include <memory>
#endif
...
static PyObject* const from_dlpack_str =
        PyUnicode_InternFromString("from_dlpack");
static PyObject* const dunder_dlpack_str =
        PyUnicode_InternFromString("__dlpack__");
static PyObject* const dl_device_str =
        PyUnicode_InternFromString("dl_device");
static PyObject* const copy_str =
        PyUnicode_InternFromString("copy");
static PyObject* const max_version_str =
        PyUnicode_InternFromString("max_version");
...
static PyObject *nb_ndarray_dlpack(PyObject *self, PyObject *const *args,
    ...
    for (Py_ssize_t i = 0; i < nkwargs; ++i) {
        ...
        if (key == dl_device_str || key == copy_str)
            continue;
        if (key == max_version_str ||
            PyObject_RichCompareBool(key, max_version_str, Py_EQ) == 1) {
...
#else
static managed_dltensor_versioned* make_mt_from_buffer_protocol(PyObject *o,
                                                                bool ro) {
#endif
    ...
    int64_t* shape = (ndim > 0) ? (int64_t*) (mt.get() + 1) : nullptr;
    int64_t* strides = (ndim > 0) ? shape + ndim : nullptr;
...
bool ndarray_check(PyObject *o) noexcept {
    if (PyObject_HasAttr(o, dunder_dlpack_str) || PyObject_CheckBuffer(o))
        return true;
...
ndarray_handle *ndarray_import(PyObject *src, const ndarray_config *c,
    ...
    mt_unique_ptr_t mt_unique_ptr(nullptr, &mt_from_buffer_delete);
#else
    void* mt = nullptr;     // In general, can be versioned or unversioned
#endif
    bool versioned = true;  // default (e.g., returned via buffer prototol)
...
#else
        static PyObject* const kwnames = Py_BuildValue("(N)", max_version_str);
        static PyObject* const version_tuple = Py_BuildValue("(ii)",
                                                       dlpack::major_version,
                                                       dlpack::minor_version);
        PyObject* args[] = {src, version_tuple};
        Py_ssize_t nargsf = 1 | PY_VECTORCALL_ARGUMENTS_OFFSET;
        capsule = steal(PyObject_VectorcallMethod(dunder_dlpack_str, args,
                                                  nargsf, kwnames));
...
#else
            mt = make_mt_from_buffer_protocol(src, c->ro);
#endif
        }
...
#ifndef OK_TO_USE_STD_UNIQUE_PTR
        if (!capsule.is_valid())
            mt_from_buffer_delete((managed_dltensor_versioned *) mt);
#endif
        return nullptr;
    }
        ...
        if (dt.code == (uint8_t) dlpack::dtype_code::Bool) {
            ...
            switch (dt.code) {
                ...
                case (uint8_t) dlpack::dtype_code::Complex:
                    ...
                    break;
                default:
                    dt.lanes = 0;  // fail (will return nullptr below)
            }
...
PyObject *ndarray_export(ndarray_handle *th, int framework,
    ...
    if (framework == numpy::value) {
        try {
            ...
            return pkg_mod.attr(array_str)(o, arg("copy")=copy).release().ptr();
#else
            static PyObject* pkg_mod = module_import("numpy");
            static PyObject* const kwnames = Py_BuildValue("(N)", copy_str);
            PyObject* args[] = {pkg_mod, o.ptr(),
                                (copy) ? Py_True : Py_False};
            Py_ssize_t nargsf = 2 | PY_VECTORCALL_ARGUMENTS_OFFSET;
            return PyObject_VectorcallMethod(array_str, args,
                                             nargsf, kwnames);
#endif
    ...
    try {
        ...
        switch (framework) {
            case pytorch::value:
                static PyObject* const torch_str =
                        PyUnicode_FromString("torch.utils.dlpack");
                pkg_name = torch_str;
                break;
            case tensorflow::value:
                static PyObject* const tensorflow_str =
                        PyUnicode_FromString("tensorflow.experimental.dlpack");
                pkg_name = tensorflow_str;
                break;
            case jax::value:
                static PyObject* const jax_str =
                        PyUnicode_FromString("jax.dlpack");
                pkg_name = jax_str;
                break;
            case cupy::value:
                static PyObject* const cupy_str =
                        PyUnicode_FromString("cupy");
                pkg_name = cupy_str;
                break;
            case memview::value:
        ...
        if (pkg_name) {
#if PY_VERSION_HEX < 0x03090000
            o = module_::import_(pkg_name).attr(from_dlpack_str)(o);
#else
            PyObject* pkg_mod = module_import(pkg_name);
            PyObject* args[] = {pkg_mod, o.ptr()};
            Py_ssize_t nargsf = 2 | PY_VECTORCALL_ARGUMENTS_OFFSET;
            o = steal(PyObject_VectorcallMethod(from_dlpack_str, args,
                                                nargsf, nullptr));
#endif
        }
    } catch (const std::exception &e) {
    ...
    if (copy) {
        PyObject* copy_function_name = copy_str;
        if (framework == pytorch::value) {
            static PyObject* const clone_str =
                    PyUnicode_FromString("clone");
            copy_function_name = clone_str;
        }
