...
public:
    ...
    int64_t* shape_ptr() const { return &m_dltensor.shape[0]; }
    int64_t* stride_ptr() const { return &m_dltensor.strides[0]; }
    ...
    size_t size() const {
        size_t ret = 1;
        for (size_t i = 0; i < m_dltensor.ndim; ++i) {
            ret *= m_dltensor.shape[i];
        }
        return ret;
    }
