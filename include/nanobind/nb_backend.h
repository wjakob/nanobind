...
/// Opaque record holding the backend state of one domain (see NB_DOMAIN):
/// type/function registries, instance maps, caches, and so on. Extensions
/// hold a pointer to their domain's state and pass it to backend functions
/// that need it; the layout is private to the backend.
struct nb_internals;
